...
short_description: Sends config from file to EOS
description:
  - The eos_command module provides a module for sending configs
    from a file to the EOS node and returns the ouput.  Commands can
version_added: 1.0.0
category: System
author: Mike Biancaniello
requirements:
...
options:
  file:
    description:
      - Specifies the filename of the configuration to load
    required: true
    default: null
    choices: []
    aliases: []
...
- name: load ntp config
  eos_install_config: file='ntp.conf'
...
def run_commands(module):
    filename = module.attributes['file']
